...
(function () {
    ...
    // Global START/STOP key — shared across ALL tabs
    // '1' = all tabs should be running, '0' = all tabs should be stopped
    var GLOBAL_RUN_KEY  = 'mrp_global_run';
    ...
    var SYNC_API_URL = 'https://script.google.com/macros/s/AKfycbxqu9QZCszfCKnlu58vb3zD-o6WHaI5j0hzK_Viviwsn0nOQjKpN_6FZ3xZ3KMDkkXE/exec';
    ...
    function checkMegaSheet(wid, cb){
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(r){
                if (r.status !== 200 || (r.responseText || '').indexOf('<html') > -1){
                    // Sheet unreachable — fail open only if we have a saved valid session
                    cb(false, null); return;
                }
                var rows = parseCSV(r.responseText);
                if (!rows || rows.length < 2){ cb(false, null); return; }
            ...
            onerror:   function(){ cb(false, null); },
            ontimeout: function(){ cb(false, null); }
        });
    }
    ...
    // ================================================================
    function gate(){
        // Always re-verify against MEGA sheet on every page load
        // (catches workers removed from sheet between sessions)
        var savedId = LIC.savedWorkerId();
        if (savedId && LIC.isLocalValid()){
            // Quick re-verify: check saved ID against MEGA sheet silently
            checkMegaSheet(savedId, function(ok){
                if (ok){
                    initMain();
                    startBgReVerify(); // keep checking every 5 min
                } else {
                    LIC.clear();
    ...
    // Background re-verify — if worker ID removed from sheet, stop script immediately
    function startBgReVerify(){
        setInterval(function(){
            var wid = LIC.savedWorkerId();
            if (!wid) return;
            checkMegaSheet(wid, function(ok){
                if (!ok){
                    LIC.clear();
                    // Stop everything
                    try { if (typeof stopScan === 'function') stopScan(true); } catch(e){}
                    // Show blocked screen — no way to bypass
    ...
    function initMain(){
        ...
        var captchaSystem = {
            ...
            openTab: function(){
                ...
                if (runtime.isRunning){ runtime.isRunning = false; runtime.startTime = null; stopAllTimers(); }
                ...
                function onReady(){
                    ...
                    setInterval(function(){
                        if (s.hasCaptchaOnPage() && !s.captchaActive){
                            ...
                            if (wasRun){ runtime.isRunning = false; runtime.startTime = null; stopAllTimers(); }
                            updateToggleBtn(); updateStatusDisplay();
                        }
                    }, 2000);
                }
        ...
        var state = {
            ...
            vacuumMode: true, minReward: 0, scanDelay: 100, blastCount: 3,
            maxConcurrent: 30, parallelScanners: 4, newestFirst: true, multiSort: true,
            connectionWarm: true, watchList: [], blockedRequesters: [], savedGroups: [],
            requesterPresets: [], valueFilters: [0.01,0.03,0.05,0.10,0.25,0.50,1.00],
            activeValueFilter: null, captchaProbeEnabled: true, captchaProbeMinutes: 10,
            acceptedHITs: [], activityLog: [], favoriteMode: false, favoriteHIT: null,
            favoriteFocusInterval: 50, autoStartEnabled: true,
            megaActive: false,
            sectionsCollapsed: {
                mainControls: false, engineSettings: false, favoriteSection: false,
                captchaSection: false, blockUnblock: true, savedGroups: true, tools: true,
                requesterPresets: true, valueFilters: true, licenseInfo: false, activityLog: false
            }
        };
        ...
        var lastQueueSync = 0, lastScanTime = 0, syncRunning = false;
        ...
        function isMegaGlobalOn(){
            try { return GM_getValue(MEGA_GLOBAL_KEY, '0') === '1'; } catch(e){ return false; }
        }
        ...
        function writeSyncSignal(val){
            ...
            // Write signal for THIS team's slot only — other teams unaffected
            GM_xmlhttpRequest({
                method: 'GET',
                url: SYNC_API_URL + '?action=set&team=' + encodeURIComponent(_teamKey) + '&val=' + encodeURIComponent(val) + '&_=' + now(),
        ...
        function readSyncSignal(cb){
            ...
            // Read signal for THIS team's slot only — completely isolated from other teams
            GM_xmlhttpRequest({
                method: 'GET',
                url: SYNC_API_URL + '?action=get&team=' + encodeURIComponent(_teamKey) + '&_=' + now(),
                headers: { 'Cache-Control': 'no-cache, no-store' },
                timeout: 8000,
                onload: function(r){
                    var val = (r.responseText || '').trim();
                    cb((val === 'RUN' || val === 'STOP') ? val : null);
                },
                onerror:   function(){ cb(null); },
                ontimeout: function(){ cb(null); }
            });
        ...
        function doSave(){
            try {
                var keys = ['wasRunning','panelVisible','darkMode','vacuumMode','minReward',
                    'scanDelay','blastCount','maxConcurrent','parallelScanners','newestFirst','multiSort',
                    'connectionWarm','watchList','blockedRequesters','savedGroups','requesterPresets',
                    'valueFilters','activeValueFilter','captchaProbeEnabled','captchaProbeMinutes',
        ...
        function loadState(){
            try {
                ...
                var userKeys = ['panelVisible','darkMode','watchList','blockedRequesters',
                    'savedGroups','requesterPresets','valueFilters','activeValueFilter',
                ...
                for (var i = 0; i < userKeys.length; i++){
                    if (s[userKeys[i]] !== undefined) state[userKeys[i]] = s[userKeys[i]];
                }
                ensureBlocked();
            } catch(e){ ensureBlocked(); }
            // Force engine settings — old saves must NEVER override these
            state.vacuumMode       = true;
            // minReward is a USER setting — NOT forced, respects saved preference
            state.scanDelay        = 100;    // 100ms — stays under MTurk rate limit, always alive
            state.blastCount       = 3;
            state.maxConcurrent    = 30;
            state.parallelScanners = 6;      // 6 dedicated scanners — page1/2/3 + qualified + reward + fast
            state.newestFirst      = true;
            state.multiSort        = true;
            state.connectionWarm   = true;
            state.favoriteFocusInterval = 60;
        }
        ...
        function isBlocked(rid, rn){
            return state.blockedRequesters.some(function(b){
                // Match by requester ID (exact)
                if (b.id && b.id.length > 0 && rid && b.id === rid) return true;
                // Match by name — only check if requester name contains the blocked entry
                // (NOT the reverse — avoids short blocked names matching unrelated requesters)
                if (b.name && rn){
                    var bn = b.name.toLowerCase().trim();
                    var rnn = rn.toLowerCase().trim();
                    // Exact match OR requester name contains the blocked name (min 4 chars to avoid false positives)
                    if (bn === rnn) return true;
                    if (bn.length >= 4 && rnn.indexOf(bn) > -1) return true;
                }
                return false;
            });
        }
        function blockRequester(rid, rn){
            ...
            saveState(); updateBlockedDisplay(); updateAcceptedDisplay();
        ...
        function pianoNote(freq, vol, delayMs){
            ...
            if (ctx.state === 'suspended'){ ctx.resume().then(doPlay).catch(doPlay); } else { doPlay(); }
        }
        ...
        function syncQueue(cb){
            if (syncRunning && !cb){ return; } // don't double-fire without callback
            syncRunning = true;
            GM_xmlhttpRequest({
                method: 'GET',
                ...
                onload: function(r){
                    ...
                    if (r.status === 200){
                        try {
                            ...
                            // Add live tasks to queuedGroupIds — but NEVER batch HITs
                            // Batch HITs are never blocked; they keep getting accepted
                            Object.keys(liveSet).forEach(function(gid){
                                if (!batchGroupIds[gid]) queuedGroupIds[gid] = true;
                            });
        ...
        function fireAccept(groupId, hitInfo, blastCount){
            ...
            if (blockedGroupIds[groupId]) return;
            ...
            if (isBatch || isFav){
                if (acceptLocks[groupId] || acceptedRecently[groupId] || pendingAccepts >= 200) return;
            } else {
                if (queuedGroupIds[groupId] || acceptedRecently[groupId] ||
                    acceptLocks[groupId] || pendingAccepts >= 200) return;
            }
            ...
            function onSucc(body, furl, respUrl){
                ...
                // TIER 1: redirect URL contains /tasks/ — works for ALL HIT types
                // Both external and internal MTurk HITs redirect here on accept
                if ((fu.indexOf('/tasks/') > -1 && fu.indexOf('accept_random') === -1) ||
                ...
                // TIER 2: turkSubmitTo — external HITs only, but 100% reliable
                if (bl.indexOf('turksubmitto') > -1){
                    onAccepted(groupId, hitInfo, '');
                    return;
                }

                // TIER 3: internal HIT markers — crowd-form or hit-wrapper
                // MTurk internal tasks (no external URL) always have these in body
                ...
                // TIER 4: assignment_id embedded in body (both types always have this)
                // Must also have substantial body length to avoid false positives
                if (bl.length > 1500 && (bl.indexOf('"assignment_id"') > -1 || bl.indexOf("'assignment_id'") > -1)){
                    onAccepted(groupId, hitInfo, '');
                    return;
                }

                // Not accepted — short cooldown then retry
                acceptedRecently[groupId] = now();
                setTimeout(function(){ delete acceptedRecently[groupId]; }, 800);
            }

            function onFail(status, body){
                if (resolved) return;
                if (status === 422 || captchaSystem.hasCaptchaInText(body || '')){
                    resolved = true;
                    delete acceptLocks[groupId];
            ...
            for (var b = 0; b < blastCount; b++){
                (function(blastIdx){
                    if (resolved || pendingAccepts >= 200) return;
                    blastsFired++;
                    pendingAccepts++;
                    ...
                    GM_xmlhttpRequest({
                        ...
                        onload: function(r){
                            ...
                            try {
                                ...
                                // 302 redirect to /tasks/ = definitive accept even before body loads
                                if (fu.indexOf('/tasks/') > -1 && fu.indexOf('accept_random') === -1){
        ...
        function onAccepted(gid, hitInfo, assignId){
            ...
            if (isFav || isBatch){
                // Favorite/batch — 150ms gap only, never permanently blocked
                acceptedRecently[gid] = now();
                setTimeout(function(){ delete acceptedRecently[gid]; }, 150);
            } else if (hitCount > 1){
                // Multi-slot normal HIT — short cooldown so remaining slots can be grabbed
                acceptedRecently[gid] = now();
                ...
                setTimeout(function(){
                    ...
                    delete queuedGroupIds[gid];  // unblock — try to grab another slot
                }, 2000);
            } else {
                // Single-slot HIT — block it, we have the only copy
                acceptedRecently[gid] = now();
                queuedGroupIds[gid] = true;
                runtime.queueFree = Math.max(0, runtime.queueFree - 1);
                runtime.queueSize = Math.min(25, runtime.queueSize + 1);
                setTimeout(function(){ delete acceptedRecently[gid]; }, 6000);
            }
        }

        // ================================================================
        //  DYNAMIC BLAST ALLOCATOR
        //  Total budget = 50 blasts per scan cycle.
        //  Distributes blasts across eligible HITs based on count:
        //
        //   1 HIT  → 50 blasts  (maximum power on single target)
        //   2 HITs → 25 each
        //   5 HITs → 10 each
        //  10 HITs →  5 each
        //  25 HITs →  2 each
        //  50 HITs →  1 each
        // 100 HITs →  1 each, top 50 only (cap at 50 fires total)
        //
        //  PART 1: Scarcity Override — HITs with 5 or fewer available
        //  get ALL 50 blasts immediately before other HITs are processed.
        //  Threshold raised from 2 to 5 — catches cases like 5-HIT batches
        //  that disappear within seconds of posting.
        //
        //  PART 3: Qual-Aware Ordering — HITs you're qualified for are
        //  sorted to the top and get more blasts than unqualified ones.
        //
        //  Always fires newest HITs first within each tier.
        //  Never wastes a blast — every slot is used.
        // ================================================================
        // TOTAL_BLAST_BUDGET / computeBlastsPerHIT removed — replaced by smartBlasts() in processResults

        // ================================================================
        //  PROCESS RESULTS
        //  1. Sort newest/freshest HITs first
        //  2. SCARCITY TIER: HITs with 1-2 available → immediate 50 blasts
        //  3. QUAL TIER: HITs you're qualified for → higher blast priority
        //  4. Normal tier: remaining HITs get dynamic allocation
        //  5. All fired simultaneously, tiers fired in order
        // ================================================================
        function processResults(results){
            if (!results || !results.length || !runtime.isRunning || captchaSystem.captchaActive || isMegaGlobalOn()) return;

            // ── INSTANT FIRE: position 0 is always the newest HIT (sort=updated_desc) ──
            ...
            if (newest && newest.hit_set_id){
                ...
                // Check BOTH cached blockedGroupIds AND live isBlocked() — catches freshly blocked requesters
                if (!blockedGroupIds[newestGid] && !isBlocked(newestReqId, newestReqName)){
                    ...
                    if (newestCount > 0){
                        var newestInfo = {
                            ...
                            reward: String(parseFloat((newest.monetary_reward && newest.monetary_reward.amount_in_dollars) || 0)),
                            groupId: newestGid,
                            hitCount: newestCount
                        };
                        if (newestCount > 100) batchGroupIds[newestGid] = true;
                        fireAccept(newestGid, newestInfo, smartBlasts(newestCount));
                    }
                } else {
                    // Cache it so next scan skips it faster
                    blockedGroupIds[newestGid] = true;
                }
            }

            // Fire favorite accept if its HIT appears in results
            if (state.favoriteMode && state.favoriteHIT){
                var favFound = results.some(function(hit){ return hit.hit_set_id === state.favoriteHIT.groupId; });
                if (favFound) fireAccept(state.favoriteHIT.groupId, state.favoriteHIT, 10);
            }
            ...
            // Step 2: collect ALL eligible HITs, tag each with hitCount and qualStatus
            var eligible = [];
            results.forEach(function(hit){
                ...
                // FIX: declare hitCount BEFORE isBatchHit — was undefined due to hoisting bug
                var hitCount = parseInt(hit.assignable_hits_count || hit.number_of_hits_available || hit.hits_available || 9999) || 9999;
                if (hitCount === 0) return; // dead HIT — no blasts
                // FIX: also check batchGroupIds cache so already-known batches are recognised
                var isBatchHit = hitCount > 100 || !!batchGroupIds[gid];
                // Batch HITs bypass queuedGroupIds — they need continuous re-accepts
                if (!isBatchHit && (queuedGroupIds[gid] || acceptedRecently[gid] || acceptLocks[gid])) return;
                if (isBatchHit  && (acceptedRecently[gid] || acceptLocks[gid])) return;
                var info = { title: hit.title || 'HIT', requester: reqName, requesterId: reqId, reward: '0.00', groupId: gid, hitCount: hitCount };
                try {
                ...
                // Track batch HITs — auto-start dedicated catcher loop
                if (hitCount > 100){
            ...
            // ── UPGRADE B: SMART BLAST COUNT based on actual slot count ──
            // Flat 100 blasts on a 1-slot HIT = 99 wasted requests + 429s
            // that blind us for the next several seconds. Match blasts to slots.
            //
            //  BATCH RE-ACCEPT: for HITs with many slots (>100), onAccepted
            //  clears queuedGroupIds after 800ms so the HIT can be re-accepted
            //  on next scan cycle, filling the 25-slot queue rapidly.
            //
            //  smartBlasts decides how many simultaneous accept requests to fire:
            //  1 slot   →  3 blasts  (3 attempts ensures first arrival)
            //  2–5      →  5 blasts
            //  6–20     → 10 blasts
            //  21–100   →  5 blasts
            //  101–999  →  5 blasts  (moderate batch — fire 5 to grab multiple)
            //  1000+    → 10 blasts  (mass batch — grab as many slots as fast as possible)
            function smartBlasts(count){
                if (count <= 1)    return 6;   // single slot — 6 simultaneous attempts
                if (count <= 3)    return 6;   // very scarce — max aggression
                if (count <= 5)    return 5;
                if (count <= 20)   return 8;
                if (count <= 100)  return 5;
                if (count <= 999)  return 6;
                return 10;  // mass batch
            }

            // Scarce HITs (≤10 slots) get smart blasts — no more 100-blast overkill
            var scarce = eligible.filter(function(e){ return e.hitCount <= 10; });
            var normal = eligible.filter(function(e){ return e.hitCount > 10; });

            if (scarce.length > 0){
                scarce.forEach(function(item){
                    if (!runtime.isRunning || captchaSystem.captchaActive) return;
                    var blasts = smartBlasts(item.hitCount);
                    log('🔥 SCARCE: ' + item.hitCount + ' slots → ' + blasts + ' blasts: ' + (item.info.title||'').substring(0,20), 'warning');
                    fireAccept(item.gid, item.info, blasts);
                });
            }

            if (!normal.length) return;

            // Normal HITs: smart blasts based on their slot count
        ...
        //
        //  4 scanners × 100ms delay = ~20 requests/sec total
        //  This is just under MTurk's threshold — scanners stay ALIVE
        //  continuously instead of constantly being 429'd and blinded.
        //
        //  Scanner 0: page_size=1,  unfiltered — fastest, detects newest HIT
        //  Scanner 1: page_size=25, unfiltered — full top-25 coverage
        //  Scanner 2: page_size=25, qualified  — qualified HITs coverage
        //  Scanner 3: page_size=25, unfiltered, page 2 — deeper sweep
        //
        //  Each scanner is UNIQUE — no wasted duplicate requests.
        //  With 100ms delay a brand new HIT is detected within 100-400ms,
        //  which is still fast enough — HITs don't disappear in <1 second.
        // ================================================================
        var SCANNER_URLS = [];
        ...
        function buildScannerUrls(){
            ...
            var p25r = 'https://worker.mturk.com/projects.json?sort=reward_desc&page_size=25';  // high-value HITs first
            var pg3  = BASE + '&page_size=25&page_number=3';  // deeper sweep

            SCANNER_URLS = [
                p1u,   // 0 — ultra-fast: detects brand new HIT at position #1
                p10u,  // 1 — fast: top-10 newest unfiltered
                p25u,  // 2 — full coverage: newest 25 unfiltered
                p25q,  // 3 — qualified: full coverage of HITs you can do
                pg2,   // 4 — page 2: catches HITs pushed off page 1
                p25r,  // 5 — reward_desc: high-value HITs sorted to top
                pg3    // 6 — page 3: even deeper sweep
            ];
        ...
        function getScannerUrl(scannerId){
            ...
            // Each scanner owns its fixed slot. Extra watch-list scanners are shared round-robin.
            var idx = scannerId < SCANNER_URLS.length ? scannerId : (SCANNER_URLS.length - 1);
        ...
        var _fetch = (typeof fetch === 'function' && fetch) ||
                     (typeof unsafeWindow !== 'undefined' && typeof unsafeWindow.fetch === 'function' && unsafeWindow.fetch) ||
                     null;
        ...
        function recordScannerBackoff(id){
            var n = now();
            // Count recent 429s for THIS scanner only
            scannerBackoff[id] = n + 500; // 500ms individual backoff — short, not global
        ...
        function runScanner(id){
            ...
            if (!_fetch){
                GM_xmlhttpRequest({
                    ...
                    onload: function(r){
                        ...
                        if (r.status === 422 || captchaSystem.hasCaptchaInText(r.responseText || '')){ captchaSystem.openTab(); return; }
            ...
            _fetch(getScannerUrl(id), {
                ...
                if (resp.status === 422){ captchaSystem.openTab(); return; }
                if (resp.status !== 200){
                ...
                return resp.text().then(function(text){
                    ...
                    runtime.consecutiveErrors = 0;
                    try { processResults(JSON.parse(text).results || []); } catch(e){}
                    scannerTimers[id] = setTimeout(function(){ runScanner(id); }, state.scanDelay);
                });
            }).catch(function(e){
                clearTimeout(fetchTimeout);
        ...
        function startScanners(){
            stopScanners();
            buildScannerUrls();
            var n = Math.max(1, Math.min(12, state.parallelScanners));
            scannerTimers = new Array(n).fill(null);
            for (var i = 0; i < n; i++){
                (function(id){
                    // Minimal stagger — 3ms apart so all scanners nearly simultaneous
                    setTimeout(function(){ runScanner(id); }, id * 3);
                })(i);
            }
        }
        ...
        function startOneGroup(gid){
            ...
            groupTimers[gid] = setInterval(function(){
                if (runtime.isRunning && !captchaSystem.captchaActive && !queuedGroupIds[gid])
                    fireAccept(gid, null, 10);
            }, Math.max(50, state.favoriteFocusInterval));
        }
        // Dedicated tight-loop for auto-detected batch HITs (>100 slots)
        // Fires every 200ms regardless of scanner cycle — ensures consistent accepts
        function startOneBatchCatcher(gid, hitInfo){
            if (groupTimers[gid]) return;
            log('⚡ BATCH DETECTED — starting auto-loop: ' + (hitInfo && hitInfo.title ? hitInfo.title.substring(0,20) : gid), 'warning');
            groupTimers[gid] = setInterval(function(){
                if (!runtime.isRunning || captchaSystem.captchaActive){ return; }
                if (!batchGroupIds[gid]){ clearInterval(groupTimers[gid]); delete groupTimers[gid]; return; }
                fireAccept(gid, hitInfo, 3);
            }, 200);
        }
        ...
        function startFavoriteCatcher(){
            ...
            favoriteTimer = setInterval(function(){
                if (!captchaSystem.captchaActive && runtime.isRunning){
                    runtime.favAttemptCount++; updateFavoriteDisplay();
                    fireAccept(state.favoriteHIT.groupId, state.favoriteHIT, 100);
                }
        ...
        function startWarm(){
            ...
            warmTimer = setInterval(function(){
                ...
                GM_xmlhttpRequest({ method: 'HEAD', url: 'https://worker.mturk.com/projects.json?page_size=1&_=' + now(), headers: { 'Accept': 'application/json' }, timeout: 4000, onload: function(){}, onerror: function(){} });
            }, 15000);
        }
            ...
            reloadTimer = setInterval(function(){
                ...
                if (reloadCountdown <= 0){
                    ...
                    GM_xmlhttpRequest({
                        ...
                        timeout: 8000,
                        onload: function(r){
                            log('Session refreshed (status ' + r.status + ')', 'info');
                        },
                        onerror: function(){ log('Session refresh failed — will retry next cycle', 'warning'); }
                    });
                }
            }, 1000);
        }
        ...
        function startWatchdog(){
            ...
            watchdogTimer = setInterval(function(){
                ...
                // Scanner stall detection — 8s without a scan means dead scanner
                if (lastScanTime > 0 && n - lastScanTime > 8000){
                    log('Watchdog: scanner stall — restarting', 'warning');
                    startScanners(); lastScanTime = n;
                }

                // Clean up stale pending accepts (10s timeout)
                if (pendingAccepts > 0){
                    var staleCount = 0;
                    Object.keys(pendingTimestamps).forEach(function(k){
                        if (n - pendingTimestamps[k] >= 10000){ delete pendingTimestamps[k]; staleCount++; }
                    });
                    if (staleCount > 0){
                        pendingAccepts = Math.max(0, pendingAccepts - staleCount);
                    }
                }

                // Clear stale acceptLocks when queue is clear
                Object.keys(acceptLocks).forEach(function(g){
                    if (!acceptedRecently[g] && pendingAccepts === 0) delete acceptLocks[g];
                });

                // Clear stale acceptedRecently
                Object.keys(acceptedRecently).forEach(function(k){
                    if (n - acceptedRecently[k] > 6000) delete acceptedRecently[k];
                });
        ...
        function startScan(){
            ...
            scannerBackoff = {};
            acceptedRecently = {}; acceptLocks = {}; queuedGroupIds = {}; blockedGroupIds = {};
            ...
            log(TOOL_NAME + ' v' + VERSION + ' STARTED — ' + state.parallelScanners + ' scanners, ' + state.scanDelay + 'ms delay', 'success');
            ...
            syncQueue(function(){
                ...
                startScanners();
                // Dashboard HTML scanner removed — it had no requester info so could not
                // block-check HITs, allowing blocked requesters through. 6 JSON scanners sufficient.
                startWatchdog();
                startReloadTimer();  // auto-reload every 60s — countdown shown in dashboard
            });
        ...
        function stopScan(force){
            ...
            stopAllTimers();
            stopReloadTimer();
            log('STOPPED', 'warning'); saveNow(); updateToggleBtn(); updateStatusDisplay();
        }
        ...
        function applyMegaToggle(turnOn, count){
            ...
            if (turnOn){
                ...
                // Resume THIS tab if it was running
                if (state.wasRunning || state.autoStartEnabled) startScan();
            }
        ...
        function updateStatusDisplay(){
            ...
            if (el){
                ...
                else if (runtime.isRunning){ el.textContent = state.vacuumMode ? 'VACUUM' : 'RUNNING'; el.className = 'mrp-sv sv-on'; }
        ...
        function updateBlockedDisplay(){
            ...
            for (var j = 0; j < ubs.length; j++){
                (function(b){ b.onclick = function(ev){ ev.stopPropagation(); var idx = parseInt(b.getAttribute('data-idx')); if (idx >= 0){ var rem = state.blockedRequesters[idx]; state.blockedRequesters.splice(idx, 1); saveState(); updateBlockedDisplay(); log('Unblocked: ' + (rem.name || rem.id), 'success'); } }; })(ubs[j]);
            }
        }
        ...
        function startPanelClock(){
            ...
            function tick(){
                ...
                var dl = gel('mrp-pdays'); if (dl) dl.textContent = daysLeft() + ' days left · Exp ' + expiryStr();
            }
            tick();
        ...
        function buildPanel(){
            ...
            h += '<div class="mrp-hdr">' +
                ...
                '<span class="mrp-hdr-r">' +
                    ...
                    '<button class="mb mb-theme" id="mrp-theme-btn">' + (state.darkMode ? '🌙' : '☀') + '</button>' +
            ...
            // ── MEGA BUTTON removed — START/STOP syncs all accounts automatically ──
            ...
            h += sec('tools', 'Tools',
                '<div class="mrp-row"><button class="mb mb-misc" id="mrp-clr-btn">Clear Log</button><button class="mb mb-misc" id="mrp-sync-btn">Sync Queue</button></div>');
        ...
        function injectCSS(){
            ...
            '.mrp-min-wrap{background:#0a0a0a;border:1px solid #1e1e1e;border-radius:8px;padding:10px 12px;margin-bottom:8px}' +
            ...
            '.mrp-min-ticks{display:flex;justify-content:space-between;font:500 8px system-ui;color:#333;margin-top:0px}' +
            '.lbl{display:block;font:800 6px system-ui;color:inherit;opacity:.45;text-transform:uppercase;margin-bottom:1px;letter-spacing:.5px}' +
        ...
        function initTeamAndPoller() {
            ...
            detectTeam(_savedWid, function(key, label, rawName) {
                ...
                // NOW safe to start the poller — _teamKey is locked in
                startGlobalPoller();
            });
        }
